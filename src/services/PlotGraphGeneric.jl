...
function plot3d!(
  dfg::AbstractDFG;
  tag = :POSE,
  labels::AbstractVector{Symbol} = sortDFG(ls(dfg; whereTags = ⊇([tag;]))), # FIXME, better support for multiple trajectories via tags
  solveKey = :default,
  fig = Figure(),
  ...
  _getPos(::VelPos3, p::ArrayPartition) = p.x[2]
  ...
  ax1 = Axis3(fig[1, 1]; title, aspect, xlabel, ylabel, zlabel)
  
  _ppes = map(labels) do lb
    var = getVariable(dfg, lb)
    _getPos(getStateKind(var), _meanPoint(getState(var, solveKey)))
  end

  @cast ppes[i,d] := _ppes[i][d]

  scatter!(ax1, ppes[:,1],ppes[:,2],ppes[:,3]; label = "poses ($(length(labels)))")
  if drawTrajectory
    lines!(ax1, ppes[:,1],ppes[:,2],ppes[:,3]; label = "trajectory")
  end

  if drawTrajectoryMarkers
    scatter!(ax1, Point3f(ppes[1,1:3]...), markersize = 20, markerspace = :pixel, marker = '✪', color = :green, label = "start $(labels[1])")
    scatter!(ax1, Point3f(ppes[end,1:3]...), markersize = 20, markerspace = :pixel, marker = '⊙', color = :red, label = "end $(labels[end])")
  end
  ...
  if drawTriads
    for var in getVariable.(dfg, labels)
      ps = _meanPoint(getState(var, solveKey))
      push!(pos, Point3f(_getPos(getStateKind(var), ps)))
      rot = _getRot(getStateKind(var), ps)
  ...
  legend && axislegend(ax1; merge = true, unique = true)
